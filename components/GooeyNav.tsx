  ...
  return (
    <>
      <style>{`
        ...
        .gooey-effect.text.active { color: white; }
        .gooey-effect.filter {
        ...
        .gooey-li.active { color: white; text-shadow: none; }
        .gooey-li::after {
          content: "";
          position: absolute;
          inset: 0;
      ...
      `}</style>
      <div className="relative" ref={containerRef} style={{ transform: 'translateZ(0)', willChange: 'transform' }}>
        <nav style={{ transform: 'translate3d(0,0,0.01px)' }}>
          <ul
            ref={navRef}
            className="flex gap-7 list-none p-0 m-0 relative z-[3]"
            style={{ color: '#aaaaaa', textShadow: '0 1px 1px hsl(205deg 30% 10% / 0.2)' }}
          >
            {items.map((item, index) => (
              <li
                key={index}
                className={`gooey-li rounded-full relative cursor-pointer text-[11px] font-bold tracking-wider uppercase text-[#aaaaaa] ${activeIndex === index ? 'active' : ''}`}
              >
                <a
                  href={item.href}
